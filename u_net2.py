...
class DEMRefinementUNet(nn.Module):
    ...
    TREINAMENTO: Entrada = 2 canais (ANADEM + GeoSampa, ambos 5m)
    INFERÊNCIA: Entrada = 1 canal (apenas ANADEM, 5m)
    ...
    def __init__(self, in_channels=2, out_channels=1, base_filters=32, depth=4, 
        ...
        # Para inferência, aceita 1 canal (só ANADEM) mas processa como se fosse 2
        # duplicando o canal ou usando padding zero
...
def generate_refined_dem(
    ...
    """
    Gera DEM refinado usando APENAS o ANADEM (sem GeoSampa)
    Este é o modo de inferência real para áreas sem dados detalhados
    """
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with rasterio.open(input_anadem_path) as src:
        ...
        for i in range(0, out_height, step):
            for j in range(0, out_width, step):
                ...
                # CRÍTICO: Duplicar o canal ANADEM para simular 2 canais
                # A rede aprenderá a extrair detalhes do próprio ANADEM
...
if __name__ == "__main__":
    ...
    input2_files = ["dados/geosampa_5m.tif"]  # GeoSampa mais detalhado
    # target_files usa input2_files por padrão (GeoSampa como ground truth)
    
    # TREINAR: usa ANADEM + GeoSampa -> aprende a refinar
    model = train_dem_refinement(
        input1_files, input2_files,
        epochs=10,
        batch_size=4,
        patch_size=128,
        save_path="model/dem_refinement_unet2.pth"
    )
    
    # INFERÊNCIA: usa APENAS ANADEM (sem GeoSampa disponível)
    # A rede aplica o conhecimento aprendido para refinar o ANADEM
    generate_refined_dem(
        "model/dem_refinement_unet2.pth",
        input_anadem_path="dados/ANADEM_Recorte_IPT_5m.tif",  # Só precisa do ANADEM!
        output_path="output/anadem_u_net2.tif",
        tile_size=256,
        overlap=0.5
    )